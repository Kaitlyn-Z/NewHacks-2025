...
from stock_data_scraping import fetch_stock_data, tickers  # your new web_scraped.py
...
def prepare_stock_data(tickers_list, days=1) -> pd.DataFrame:
    """Fetch stock data and compute volume z-score, volume ratio, alerts, RSI."""
    data_frames = []

    for t in tickers_list:
        df = fetch_stock_data([t], days)  # fetch_stock_data can accept list
        if df.empty:
            continue
        df['Ticker'] = t
...
def get_latest_active_alerts(data: pd.DataFrame, threshold_z: float = 1.5):
    """Return latest per-ticker data and active alerts above threshold."""
...
def update_active_alerts(data: pd.DataFrame, active_alerts_new: pd.DataFrame,
    ...
    """Update active_alerts table in DB. Return list of newly added tickers."""
    newly_added = []
    with sqlite3.connect(db_path) as conn:
...
def send_new_alert_emails(active_alerts_new: pd.DataFrame, newly_added: list, db_path="backend/alerts.db"):
    """Send emails for newly triggered alerts only."""
    if not newly_added:
        return
...
def update_latest_alerts_table(active_alerts_new: pd.DataFrame, db_path="backend/alerts.db"):
    """Update latest_alerts table for dashboard display."""
    with sqlite3.connect(db_path) as conn:
...
def run_alert_pipeline(days: int = 1, alert_threshold_z: float = 1.5, volume_tolerance: float = 1.5):
    """Full end-to-end pipeline: fetch data, compute alerts, update DB, send emails."""
    setup_database()
    data = prepare_stock_data(tickers, days)
    if data.empty:
        print("No stock data available. Exiting pipeline.")
        return pd.DataFrame()  # or handle empty case
    ...
    return active_alerts_new  # can be used to display on dashboard
